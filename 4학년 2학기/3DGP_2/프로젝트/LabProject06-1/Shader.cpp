...
void CObjectsShader::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		...
		CB_GAMEOBJECT_INFO *pbMappedcbGameObject = (CB_GAMEOBJECT_INFO *)(m_pcbMappedGameObjects + (j * ncbElementBytes));
...
void CObjectsShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	...
	for (int j = 0; j < m_nObjects; j++)
	{
		if (m_ppObjects[j])
		{
			pd3dCommandList->SetGraphicsRootConstantBufferView(2, d3dGpuVirtualAddress + (ncbElementBytes * j));
			m_ppObjects[j]->Render(pd3dCommandList, pCamera);
			if (m_ppObjects[j]->m_bVisible) ++visibleNumber;
		}
	}

	cout << "Visible : " << visibleNumber << endl;
}
